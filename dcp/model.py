...
class PointNet(nn.Module):
    def __init__(self, emb_dims=512, batchnorm=False):
        ...
        if batchnorm:
            ...
            self.bn1 = torch.nn.Identity(64)
            self.bn2 = torch.nn.Identity(64)
            self.bn3 = torch.nn.Identity(64)
            self.bn4 = torch.nn.Identity(128)
            self.bn5 = torch.nn.Identity(emb_dims)
...
class DGCNN(nn.Module):
    def __init__(self, emb_dims=512, batchnorm=False, device=None):
        ...
        if batchnorm:
            ...
            self.bn2 = nn.Identity(64)
            self.bn3 = nn.Identity(128)
            self.bn4 = nn.Identity(256)
            self.bn5 = nn.Identity(emb_dims)
        self.device = device

    def forward(self, x):
        batch_size, num_dims, num_points = x.size()
        x = get_graph_feature(x, device=self.device)
        x = F.relu(self.bn1(self.conv1(x)))
...
class Transformer(nn.Module):
    def __init__(self, args):
        # emb_dims, n_blocks, dropout, ff_dims, n_heads
...
class DCP(nn.Module):
    ...
    def forward(self, *input):
        ...
        :param input: Src, target, which are each [batch_size, 3, 1024]
        :return:
        """
        src = input[0]
        tgt = input[1]
